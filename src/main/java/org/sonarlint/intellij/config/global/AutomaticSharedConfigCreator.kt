    ...
    init {
        ...
        val listener: DocumentListener = object : DocumentAdapter() {
            override fun textChanged(e: DocumentEvent) {
                if (tokenField.textProperty().value.isNotEmpty()) {
                    createConnectionAction.isEnabled = true
                } else {
                    createConnectionAction.isEnabled = false
                }
            }
        }
        tokenField.document.addDocumentListener(listener)

        tokenField.addActionListener {
            println("action")
        }
        centerPanel.add(
        ...
        tokenGenerationButton.addActionListener(ActionListener { openTokenCreationPage(isSQ, serverUrl) })
        init()
    }
        ...
        try {
            val progressResult = ProgressRunner<HelpGenerateUserTokenResponse?> { pi: ProgressIndicator? ->
                computeOnPooledThread<HelpGenerateUserTokenResponse>(
                    ...
                    val future =
                        SonarLintUtils.getService<BackendService>(
                            BackendService::class.java
                        ).helpGenerateUserToken(
                            serverUrl,
                            !isSQ
                        )
                    ProgressUtils.waitForFuture<HelpGenerateUserTokenResponse>(
                        pi!!,
                        future
                    )
                }
