...
class SonarLintRulePanel(private val project: Project, parent: Disposable) : JBLoadingPanel(BorderLayout(), parent) {
    ...
    private fun updateHeader(finding: Finding, ruleDescription: EffectiveRuleDetailsDto) {
        ...
        if (finding is LiveSecurityHotspot) {
            ...
            headerPanel.updateForSecurityHotspot(project, parent, ruleDescription.key, ruleDescription.type, finding)
        } else {
            headerPanel.updateForIssue(project, parent, ruleDescription.type, ruleDescription.severity, finding as Issue)
        }
    }
