...
public class ServerDownloadProjectTask extends Task.WithResult<Map<String, SonarProject>, Exception> {
  ...
  @Override
  protected Map<String, SonarProject> compute(@NotNull ProgressIndicator indicator) throws Exception {
    try {
      return getService(BackendService.class)
        .getAllProjects(server).get()
        .getSonarProjects()
        .stream().collect(Collectors.toMap(SonarProject::getKey, p -> p));
    } catch (Exception e) {
