  ...
  private class IssueUpdater {
    ...
    private void downloadAllServerIssues(Module module, String projectKey) {
      try {
        ...
        engine.downloadAllServerIssues(server.getEndpointParams(), getService(BackendService.class).getBackend().getHttpClient(server.getName()), projectKey, branchName, null);
    ...
    private List<ServerIssue> downloadAllServerIssuesForFile(Module module, String relativePath) {
      ...
      try {
        engine.downloadAllServerIssuesForFile(server.getEndpointParams(), getService(BackendService.class).getBackend().getHttpClient(server.getName()), projectBinding, relativePath, branchName, null);
      } catch (DownloadException e) {
        var console = getService(myProject, SonarLintConsole.class);
        console.info(e.getMessage());
      }
