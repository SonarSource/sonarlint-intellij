...
class BackendService : Disposable {
    ...
    private fun startSloopProcess(): Sloop {
        ...
        return sloopLauncher.start(
            sloopPath,
            jreHomePath,
            "-Xms384m -XX:+UseG1GC -XX:MaxGCPauseMillis=200"
        )
    }
    ...
    fun moduleRemoved(module: Module) {
        ...
        // Guard: Don't remove if project is disposed (cleanup will happen via projectClosed)
        if (project.isDisposed) {
            GlobalLogOutput.get().log("Skipping module removal for '$moduleId' - project is disposed", ClientLogOutput.Level.DEBUG)
            return
        }
        
        // Guard: Verify the module is actually gone from the project
