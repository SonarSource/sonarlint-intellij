...
object ProblemsUtils {
    fun createProblemDescriptorForPmdRule(
        ...
        if (psiFile.virtualFile.canonicalPath!!.endsWith(".vm") || psiFile.virtualFile.canonicalPath!!.endsWith(".kt")) {
        ...
        return manager.createProblemDescriptor(
            psiElement, endElement,
            desc, ProblemHighlightType.GENERIC_ERROR, isOnTheFly,
            quickFix(psiElement)
        )
    }
    ...
    private fun createTextRangeProblem(
        ...
        return manager.createProblemDescriptor(
            psiFile, textRange,
            desc, ProblemHighlightType.GENERIC_ERROR,
            isOnTheFly, quickFix()
        )
    }
}
