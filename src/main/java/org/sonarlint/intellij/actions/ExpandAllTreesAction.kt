...
/**
 * Action to expand all trees in the SonarQube for IDE tool window.
 * Works for both Current File and Report tabs.
 */
class ExpandAllTreesAction : AnAction("Expand All", "Expand all finding trees", AllIcons.Actions.Expandall), DumbAware {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        runOnUiThread(project) {
            ...
            when (val component = selectedContent.component) {
                is CurrentFilePanel -> {
                    expandCurrentFileTrees(component)
                }
                is ReportPanel -> {
                    expandReportTrees(component)
                }
            }
        }
    }
    
    private fun expandCurrentFileTrees(panel: CurrentFilePanel) {
        panel.expandAllTrees()
    }
    
    private fun expandReportTrees(panel: ReportPanel) {
        panel.expandAllTrees()
    }
}
