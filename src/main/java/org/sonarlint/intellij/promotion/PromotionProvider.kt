...
class PromotionProvider(private val project: Project) {
    ...
    private fun processFullProjectPromotion(notifications: SonarLintProjectNotifications) {
        ...
        if (!wasReportEverUsed && firstAutoAnalysis.isBefore(Instant.now().minus(FULL_PROJECT_PROMOTION_DURATION))) {
            showPromotion(notifications, "Detect issues in your whole project", Promotion.DETECT_PROJECT_ISSUES)
        }
    }

    private fun processFasterProjectAnalysisPromotion(notifications: SonarLintProjectNotifications) {
        showPromotion(notifications, "Speed up the project-wide analysis", Promotion.SPEED_UP_ANALYSIS)
    }

    private fun processCICDProjectAnalysisPromotion(notifications: SonarLintProjectNotifications) {
        showPromotion(notifications, "Analyze your project in your CI/CD pipeline", Promotion.ANALYZE_CI_CD)
    }
    ...
    private fun processAdvancedLanguagePromotion(notifications: SonarLintProjectNotifications, extension: String) {
        ...
        if (language != null) {
            showPromotion(notifications, "Detect more security issues in your " + language.label + " files", Promotion.DETECT_SECURITY_ISSUES)
        }
    }
    ...
    fun processExtraLanguagePromotion(languagesToPromote: Set<Language>) {
        ...
        showPromotion(notifications, "Enable $languagesText analysis by connecting your project", Promotion.ENABLE_LANGUAGE_ANALYSIS)
    }
    ...
    private fun showPromotion(notifications: SonarLintProjectNotifications, content: String, promotion: Promotion) {
        synchronized(this) {
            val lastModifiedDate: Instant? = getLastModifiedDate()

            if (shouldNotify(lastModifiedDate)) {
                notifications.notifyLanguagePromotion(content, promotion)
