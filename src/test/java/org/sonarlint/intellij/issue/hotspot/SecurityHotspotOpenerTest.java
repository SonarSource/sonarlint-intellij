...
public class SecurityHotspotOpenerTest extends AbstractSonarLintLightTests {
  ...
  public void it_should_fail_when_no_project_is_opened() {
    ...
    registerConnection(SERVER_ID, CONNECTED_URL);
  ...
  @Test
  public void it_should_fail_when_a_single_open_project_is_bound_to_another_server() {
    when(projectManager.getOpenProjects()).thenReturn(new Project[] {getProject()});
    registerConnection(SERVER_ID, CONNECTED_URL);
    getProjectSettings().setBindingEnabled(true);
    getProjectSettings().setProjectKey(PROJECT_KEY);
    getProjectSettings().setServerId("differentServer");
  ...
  @Test
  public void it_should_fail_when_a_single_open_project_is_bound_to_another_project_key() {
    when(projectManager.getOpenProjects()).thenReturn(new Project[] {getProject()});
    registerConnection(SERVER_ID, CONNECTED_URL);
    getProjectSettings().setBindingEnabled(true);
    getProjectSettings().setProjectKey("anotherProjectKey");
  ...
  private void registerConnection(String name, String hostUrl) {
    getGlobalSettings().setSonarQubeServers(Collections.singletonList(SonarQubeServer.newBuilder().setHostUrl(hostUrl).setName(name).build()));
  }
