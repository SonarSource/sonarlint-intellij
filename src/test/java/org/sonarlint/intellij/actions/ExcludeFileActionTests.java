...
class ExcludeFileActionTests extends AbstractSonarLintHeavyTests {
  ...
  void setup() {
    ...
    Awaitility.await().atMost(100, TimeUnit.SECONDS).untilAsserted(() -> assertThat(getService(myProject, AnalysisReadinessCache.class).isModuleReady(getModule())).isTrue());
