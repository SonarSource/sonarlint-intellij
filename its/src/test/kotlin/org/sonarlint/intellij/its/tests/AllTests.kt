...
class AllTests : BaseUiTest() {
    ...
    inner class BindingTest : BaseUiTest() {
        @BeforeAll
        fun prepare() {
            ORCHESTRATOR.distribution.addPluginLocation(
                requireNonNull(
                    MavenLocation.of(
            ...
            executeBuildWithSonarScanner(
                "projects/sample-scala/",
                ORCHESTRATOR,
                PROJECT_KEY
            );
            executeBuildWithSonarScanner(
                "projects/sample-scala/mod/",
                ORCHESTRATOR,
                MODULE_PROJECT_KEY
            );

            org.sonarlint.intellij.its.tests.BindingTest.token = generateToken(adminWsClient, "BindingTest")
        ...
        private fun bindProjectAndModuleInFileSettings() {
            sonarLintGlobalSettings {
                ...
                dialog("New Connection: Authentication") {
                    jPasswordField().text = org.sonarlint.intellij.its.tests.BindingTest.token
                    button("Next").click()
                }
    ...
    inner class OpenInIdeTest : BaseUiTest() {
        ...
        fun prepare() {
            ...
            OrchestratorUtils.executeBuildWithMaven(
                "projects/sample-java-hotspot/pom.xml",
                ORCHESTRATOR
            );
    ...
    inner class SecurityHotspotTabTest : BaseUiTest() {
        ...
        fun initProfile() {
            ...
            token =
                OrchestratorUtils.generateToken(adminWsClient, "SecurityHotspotTabTest")
        }
    ...
    inner class TaintVulnerabilitiesTest : BaseUiTest() {
        ...
        fun prepare() {
            ...
            // Build and analyze project to raise hotspot
            OrchestratorUtils.executeBuildWithMaven(
                "projects/sample-java-taint-vulnerability/pom.xml",
                ORCHESTRATOR
            );

            org.sonarlint.intellij.its.tests.TaintVulnerabilitiesTest.token =
                OrchestratorUtils.generateToken(adminWsClient, "TaintVulnerabilitiesTest")
        }
    ...
    inner class CurrentFileTabTest : BaseUiTest() {
        ...
        fun prepare() {
            ...
            OrchestratorUtils.executeBuildWithMaven(
                "projects/sample-java-issues/pom.xml",
                ORCHESTRATOR
            );

            token = OrchestratorUtils.generateToken(adminWsClient, "CurrentFileTabTest")
        }
